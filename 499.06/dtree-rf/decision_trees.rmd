...
# pkg_list = c('ggplot2', 'readxl', 'rpart', 'randomForest') 
...
summary(diab_model)
```
This returns a lot of information, including

* the error for different tree complexities

* scores for variable importance

* information about each node

  - the number of observations used to train the node
  
  - the probability a sample will be categorized by the node
  
  - the probability of the splits at the node if not a leaf

To return subsets of this information, we can use other functions encapsulated in the package.
...
It returns a nested list of splits, with information on the total number of samples used and categorized from the training data. This can be helpful for quickly assessing the evennessof splits.
...
The Gini Coefficient takes a value between 0 and 1 and measures both the accuracy and diffusion of classifications, with 0 representing perfect classification, 1 a perfectly random distribution. The algorithm used in `randomForest` minimizes the Gini Coefficient.


We can also visualize importance with the `varImpPlot` function.
