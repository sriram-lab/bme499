...
author: "Dr. Sriram Chandrasekaran and Joshua Sodicoff"
date: "10/21/2020"
...
library(cluster) #a utility for clustering techniques, which contains the silhouette function
...
When analyzing biological data in R, we'll mostly be dealing with large vectors. In these cases, let n be equal to the length of the vectors we're comparing (note: the vectors must have the same length). 
...
We can also use the `norm` function
...
Where `type = "2"` refers to the L2 norm.
...
`cor` has an additional `method` parameter, which results in the use of `pearson`(default), `kendall` and `spearman` correlation
...
planet_plot + 
  ...
  geom_text(label = "Gas giants", x = 1.5, y = 40000, color = "red") +
...
```{r planet_kmeans}
k_2_idents = kmeans(numeric_planet_data, centers = 2) #run k means on the numeric planet data
idents = as.factor(k_2_idents[["cluster"]]) #extract the cluster info from the output and convert to a categorical variable
planet_data = cbind(planet_data, idents = idents) #adds an additional column to planet_data with the clustering generated by kmeans
ggplot(planet_data, aes(x = density, y = rad, label = name, color = idents)) +
  geom_point(size = 5) + geom_label_repel(aes(label = name)) + 
  ggtitle("Radius and Average Density of Planets") +
  xlab("Density (g/cm^3)") +
  ylab("Radius (km)") +
...
planet_plot + 
  ...
  geom_text(label = "Gas giants", x = 1.5, y = 50000, color = "red") +
  geom_text(label = "Ice giants", x = 1.75, y = 35000, color = "purple") +
...
idents = as.factor(k_3_idents[["cluster"]]) #extract the cluster info from the output and convert to a categorical variable
planet_data = cbind(planet_data, idents_3 = idents) #adds an additional column to planet_data with the clustering generated by kmeans
...
silhouette_score <- unlist(lapply(2:5, function(k){
  k_means <- kmeans(numeric_planet_data, centers = k, nstart=25) #run kmeans for a given value of k
  silhouette_out <- silhouette(k_means$cluster, dist_planets) #generate silhouette scores for the given clustering
  sil_mat = matrix(silhouette_out,ncol = 3) #converts the output of the silhouette function into a matrix
  sil_mat = sil_mat[,3] #subsets the matrix to the silhouette scores
  return(mean(sil_mat)) #returns the mean silhouette score
}))
silhouette_df = data.frame(clusts = as.factor(2:5), score = silhouette_score) #creates a data frame with values of k and their associated silhouette scores
ggplot(silhouette_df, aes(x = clusts, y = score, fill = clusts)) +
  geom_bar(stat = "identity") +
  ggtitle("Silhouette scores for k = 2 to 5") +
  xlab("Cluster")+
  ylab("Score") + 
  theme(legend.position = "none")
```

*Note*: As the placement of cluster centroid is random for every iteration, clustering results may fluctuate for different iterations. The `kmeans` function contains additional properties that can be used to address this problem. To easily increase the stability, increase the `nstart` parameter above 1.
...
In R, we can perform hierarchical clustering using the `hclust` function, and k-means clustering using the `kmeans` function. The `silhouette` function is used to calculate the silhouette value for each observation based on its cluster assignment from k-means clust
...
subset_vars = c('HOSPMI','TOTCHOL','SYSBP','DIABP','BMI','HEARTRTE','GLUCOSE','HDLC','LDLC') #create a list of columns to subset to
fram_health = fram[,subset_vars] #subset the object to the given columns
head(fram_health) #display the first several rows of the given object
```
...
rm_fram = apply(fram_health, MARGIN = 1, function(x){
  ...
  rm_row = any(na_fram) #determines if any values are TRUE, implying the row contains NA values
  return(!rm_row) #returns if the row does not contain NA values
})
fram_health = fram_health[rm_fram,] #subset to rows without NA values
fram_health = fram_health[1:50,] #subset to the first 50 rows
hospmi_vec = as.vector(fram_health$HOSPMI) #converts the data frame HOSPMI column to a vector
heart_attack =factor(hospmi_vec); #converts the column into a categorical variable
levels(heart_attack) = c("No","Yes") #sets the category values
fram_health_data = fram_health[,2:ncol(fram_health)] #remove the HOSPMI column from the data frame
fram_health_data = as.matrix(fram_health_data) #convert the data frame to a matrix
...
```{r clustergram}
heatmap(fram_health_data, Colv=F, scale = "col",
...
hclust.col <- hclust(as.dist(1-cols.cor)) #hierarchical clustering on correlation distances 
hclust.row <- hclust(as.dist(1-rows.cor))
heatmap(fram_health_data, scale = "col",
...
df = as.data.frame(fram_chol)

silhouette_score <- unlist(lapply(2:10, function(k){
...
silhouette_df = data.frame(clusts = as.factor(2:10), score = silhouette_score)
ggplot(silhouette_df, aes(x = clusts, y = score, fill = clusts)) +
  geom_bar(stat = "identity") +
  ggtitle("Silhouette scores for k = 2 to 10") +
  xlab("Cluster")+
  ylab("Score") + 
  theme(legend.position = "none")
```
...
Let's visualize a scatter plot of our data clustered into 2 groups:
...
k_2_idents = kmeans(fram_chol_sub, centers = 2)
df = data.frame(fram_chol, Clusters = as.factor(k_2_idents$cluster))

ggplot(df, aes(x = TOTCHOL, y = LDLC, color = Clusters)) +
  geom_point(size = 2) +
  ggtitle("K Means Clustering of Framingham data, where k = 2")+
...
prop_variance = variance/sum(variance) #calculate the variance explained by each principal component
pcs = paste0("PC_",1:length(prop_variance)) #generate a vector of principal component names (PC_1, PC_2, ... )
pca_var_df = data.frame(PC = pcs, Variance = prop_variance) #create a data frame with the principal component names and variance data
...
For the example shown above, we are able to represent ~50% of the variance using the first two PCs. In other words, if we were to plot the data based on PC1 and PC2, we would be visualizing a two-dimensional representation of the data that explains ~50% of the variance. 
...
```{r pca_fram}
pca_df = data.frame(PC1 = score[,1], PC2 = score[,2], heart_attack = heart_attack) #create a data frame with the first two principal components and heart attack categorical data
...
Based on this plot, we can see that individuals who suffered from a heart attack cluster within the positive range along PC1. We can check the coefficient matrix (coeff) to see which features contribute the most for PC1:
...
* `norm` - calculates vector and matrix norms, including the L2 norm
...
* `silhouette` - calculate a silhouette score for a given clustering

* `prcomp` - runs PCA on a given numeric matrix

* `read.csv` - loads a data frame from the given file address
...
* `sum` - adds all values in a given object

* `sqrt` - takes the square root of all values in a given object

* `cbind` - combines two objects by row
...
* `data.frame` - generates a data frame, with each parameter as a column of the resulting object

* `as.data.frame` - converts the given object into a data frame

* `apply` - runs a function on each row and/or column of a list

* `is.na` - returns an object of conditionals representing if entries to the object passed are NA

* `any` - returns if any values passed are TRUE

* `!` - inverts the value of a conditionals

* `levels` - accesses the levels names attribute of an object
