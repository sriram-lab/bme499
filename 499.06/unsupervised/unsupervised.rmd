...
planet_data$idents_3 = idents_3 #adds an additional column to planet_data with the clustering generated by kmeans
...
silhouette_score <- unlist(lapply(k1:k2, function(k){
  ...
  silhouette_out <- silhouette(k_means$cluster, dist_planets) # generate silhouette values for each cluster
  ...
  return(mean(sil_mat)) # returns the mean of slihouette values (silhouette score)
}))
