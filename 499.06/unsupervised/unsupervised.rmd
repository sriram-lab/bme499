...
# pkg_list = c('ggplot2', 'ggrepel', 'cluster', 'readxl') # define packages to install
# install.packages(pkg_list) # install packages
```
...
planet_plot + 
  ...
  geom_text(label = "Rocky planets", x = 4.5, y = 40000, color = "blue")
```
...
planet_plot + 
  ...
  geom_text(label = "Ice giants", x = 1.5, y = 10000, color = "purple") +
  geom_text(label = "Rocky planets", x = 4.5, y = 30000, color = "blue")
```
...
One way to assess whether we chose a good value for k is to evaluate the mean of all silhouette values. For the purposes of this lesson, we'll refer to this mean value as the silhouette score. In general, a **silhouette score** above 0.5 indicates that the data is well-separated. The following code calculates the silhouette score (using `silhouette_score`) for our planetary data based on k-means clustering with different k values and visualizes the clustered data: 

```{r silhouette_score_function, include=FALSE}
# define a function to determine the silhouette score
silhouette_score <- function(k_list, num_data, 
                             dist_method="euclidean", cor_method="pearson") {
  unlist(lapply(k_list, function(k){
  # run kmeans for a given value of k
  k_means <- kmeans(num_data, centers = k, nstart=25) 
  # generate silhouette values for each cluster
  if (dist_method=="cor") {
    dist_data <- as.dist(1 - cor(t(num_data), use = "pairwise.complete.obs",
                                 method = cor_method))
    silhouette_out <- silhouette(k_means$cluster, dist_data) 
  }
  else {
    silhouette_out <- silhouette(k_means$cluster, 
                                 dist(num_data, method=dist_method)) 
  }
...
```{r silhouette_plot}
k1 = 2; k2 = 5 # define k range of interst as [k1, k2]

# create a data frame with values of k and their associated silhouette scores
silhouette_df = data.frame(clusts = as.factor(k1:k2), 
                           score = silhouette_score(k1:k2, numeric_planet_data))
...
# calculate silhouette scores for k range of interest
silhouette_df = data.frame(clusts = as.factor(k1:k2), 
                           score = silhouette_score(k1:k2, fram_chol))
...
* `silhouette_score` - calculate silhouette scores given a range of k values and data (**note: custom function**)
